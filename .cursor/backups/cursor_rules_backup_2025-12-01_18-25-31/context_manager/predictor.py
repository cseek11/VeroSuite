...
class ContextPredictor:
    ...
    def predict_next_tasks(self, current_task: Dict) -> List[TaskPrediction]:
        """
        Enhanced prediction using static patterns + dynamic stats + message analysis.
        ...
        # 1) Static workflow patterns (base weights)
        ...
        # 2) Dynamic transition stats (learned from history)
        ...
        # 3) Message semantic analysis (enhance existing heuristics)
        ...
        # 4) File type patterns (enhance existing conditional predictions)
    ...
    def _get_conditional_predictions(self, current_task: Dict) -> List[TaskPrediction]:
        """
        Context-aware predictions based on task outcomes.
        ...
        if current_task.get('primary_task') == 'fix_bug':
            predictions.append(TaskPrediction(
                task='run_tests',
                probability=0.85,
