...
def load_yaml(path: str) -> dict:
    ...
    Supports both old and new rubric formats:
    - Old: Simple key-value pairs (tests, bug_fix, etc.)
    - New: Nested structure with scoring/penalties/output sections
    """
    result = {
        "scoring": {},
        "penalties": {},
    ...
    try:
        with open(path, "r", encoding="utf-8") as handle:
            ...
            for i, line in enumerate(lines):
                ...
                # Detect section headers (scoring:, penalties:, output:)
                if stripped.endswith(":") and ":" in stripped and not stripped.startswith("  "):
                    section_name = stripped.rstrip(":").strip()
                    if section_name in ["scoring", "penalties", "output"]:
                        current_section = section_name
                        current_category = None
                        continue
                
                # Parse nested structure (new format)
                if current_section:
                    # Count leading spaces for indentation
                    leading_spaces = len(line) - len(line.lstrip())
                    
                    if current_section == "scoring" or current_section == "penalties":
                        # Look for category name (e.g., "search_first:")
                        if ":" in stripped and not stripped.startswith("  ") and leading_spaces == 2:
                            category_name = stripped.split(":")[0].strip()
                            current_category = category_name
                            if current_category not in result[current_section]:
                                result[current_section][current_category] = {}
                        # Look for "points:" or "description:" under category
                        elif current_category and leading_spaces >= 4:
                            if ":" in stripped:
                                ...
                                if key == "points":
                                    try:
                                        result[current_section][current_category]["points"] = int(value)
                                    except ValueError:
                                        pass
                                elif key == "description":
                                    result[current_section][current_category]["description"] = value
                    
                    elif current_section == "output":
                        # Parse output thresholds (min_pass_score, warning_score, fail_score)
                        if ":" in stripped:
                            key, value = stripped.split(":", 1)
                            key = key.strip()
                            value = value.strip()
                            if key in ["min_pass_score", "warning_score", "fail_score"]:
                                try:
                                    result["output"][key] = int(value)
                                except ValueError:
                                    pass
                
                # Fallback: Try to parse old format (simple key-value pairs)
                if ":" in stripped and not current_section:
                    key, value = stripped.split(":", 1)
                    key = key.strip()
                    value = value.strip()
                    ...
                    if key in ["tests", "bug_fix", "docs", "performance", "security"]:
                        try:
                            result["scoring"][key] = {"points": int(value)}
                        except ValueError:
                            pass
                    elif key in ["failing_ci", "missing_tests", "regression"]:
                        try:
                            result["penalties"][key] = {"points": int(value)}
                        except ValueError:
                            pass
            
            # If we got old format, convert to new structure
            if not result["scoring"] and not result["penalties"]:
