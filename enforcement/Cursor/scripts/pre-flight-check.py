...
try:
    ...
    # Fallback if enforcement package not available
    get_memory_bank_root = None
    get_rules_root = None
...
class PreFlightChecker:
    ...
    def __init__(self, project_root: Optional[Path] = None):
        ...
        # Use enforcement.config_paths if available, otherwise fallback to old paths
        if get_memory_bank_root:
            self.memory_bank_dir = get_memory_bank_root(self.project_root)
            self.rules_dir = get_rules_root(self.project_root)
        else:
            # Fallback to old paths if enforcement package not available
            self.memory_bank_dir = self.project_root / ".ai" / "memory_bank"
