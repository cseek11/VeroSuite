...
class Violation:
    ...
    line_number: Optional[int] = None  # Line number where violation occurred (1-indexed)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "line_number": self.line_number  # CRITICAL: Include line_number in JSON output
        }
...
class EnforcerReport:
    ...
    def save(self, path: Path = None, project_root: Path = None):
        """
        Save report to file.
        
        Implements dual-write strategy:
        - Full report: .ai/logs/enforcer/ENFORCER_REPORT_FULL.json
        - Summary: .cursor/enforcement/ENFORCER_REPORT.json (compatible structure)
        
        Args:
            path: Optional explicit path for summary (defaults to .cursor/enforcement/ENFORCER_REPORT.json)
            project_root: Optional project root (will be detected if not provided)
        """
        if project_root is None:
            project_root = get_project_root()
        
        # Write full report to .ai/logs/enforcer/
        full_path = get_enforcer_log_root(project_root) / "ENFORCER_REPORT_FULL.json"
        full_path.parent.mkdir(parents=True, exist_ok=True)
        full_path.write_text(self.to_json())
        # Use ASCII-safe output for Windows console compatibility
        if _use_ascii_output():
            print(f"[OK] Full report saved: {full_path}")
        else:
            print(f"✓ Full report saved: {full_path}")
        
        # Write summary to .cursor/enforcement/ (compatible structure)
        if path is None:
            summary_path = get_cursor_enforcer_root(project_root) / "ENFORCER_REPORT.json"
        else:
            summary_path = path
        
        # Add reference to full report in summary
        summary_dict = self.to_dict()
        summary_dict["full_report_path"] = str(full_path.relative_to(project_root))
    ...
    @classmethod
    def load(cls, path: Path = None, project_root: Path = None) -> Optional['EnforcerReport']:
        """
        Load report from file.
        
        Args:
            path: Optional explicit path (defaults to .cursor/enforcement/ENFORCER_REPORT.json)
            project_root: Optional project root (will be detected if not provided)
        """
        if path is None:
            if project_root is None:
                project_root = get_project_root()
            path = get_cursor_enforcer_root(project_root) / "ENFORCER_REPORT.json"
