...
class TwoBrainReporter:
    ...
    def generate_report(
        ...
        # CRITICAL: Filter out violations for log files BEFORE generating report
        ...
        for v in violations:
            if v.file_path:
                ...
                if file_path.exists() and is_log_file(file_path):
                    filtered_count += 1
                    logger.info(
                        f"Filtering stale log file violation from Two-Brain report",
                ...
                if ".ai/memory_bank/" in normalized_path or ".ai/memory-bank/" in normalized_path:
                    filtered_count += 1
                    logger.info(
                        f"Filtering stale memory_bank violation from Two-Brain report (path check)",
                        operation="generate_two_brain_report",
                        file_path=v.file_path,
                        line_number=v.line_number,
                        rule_ref=v.rule_ref,
        ...
        if filtered_count > 0:
            logger.info(
                f"Filtered {filtered_count} stale log file violations from Two-Brain report",
                operation="generate_two_brain_report",
        ...
        # Use filtered violations for report generation
        violations = filtered_violations
        
        try:
            enforcement_path = get_cursor_enforcer_root(project_root)
            ...
            report.save(project_root=project_root)
