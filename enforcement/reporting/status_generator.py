...
class StatusGenerator:
    ...
    def generate_agent_status(
        ...
        # CRITICAL: Filter out violations for log files BEFORE processing
        # This removes stale violations from previous sessions
        filtered_violations = []
        filtered_count = 0
        ...
        for v in violations:
            if v.file_path:
                ...
                if file_path.exists() and is_log_file(file_path):
                    filtered_count += 1
                    logger.info(
                        f"Filtering stale log file violation from status report",
                        operation="generate_agent_status",
                        file_path=v.file_path,
                        line_number=v.line_number,
                        rule_ref=v.rule_ref,
                        reason="log_file_violation"
                    )
                    continue
                # Also check path string (defensive check for non-existent files)
                normalized_path = str(v.file_path).replace("\\", "/").lower()
                if ".ai/memory_bank/" in normalized_path or ".ai/memory-bank/" in normalized_path:
                    filtered_count += 1
                    logger.info(
                        f"Filtering stale memory_bank violation from status report (path check)",
                        operation="generate_agent_status",
                        file_path=v.file_path,
                        line_number=v.line_number,
                        rule_ref=v.rule_ref,
                        reason="memory_bank_path_check"
                    )
                    continue
        ...
        if filtered_count > 0:
            logger.info(
                f"Filtered {filtered_count} stale log file violations from status report",
                operation="generate_agent_status",
                total_before=len(violations),
                total_after=len(filtered_violations),
                filtered_count=filtered_count
            )
        
        # Use filtered violations for all subsequent processing
        violations = filtered_violations
        
        # Summary file in .cursor/enforcement/
        status_file = enforcement_dir / "AGENT_STATUS.md"
        
        # Detect project root from enforcement_dir
        project_root = get_project_root()
        # Full status file in .ai/logs/enforcer/
        full_status_file = get_enforcer_log_root(project_root) / "AGENT_STATUS_FULL.md"
...
"""
        ...
        try:
            ...
            # Write full status to .ai/logs/enforcer/
            full_status_file.parent.mkdir(parents=True, exist_ok=True)
            with open(full_status_file, "w", encoding="utf-8") as file:
                file.write(content)
