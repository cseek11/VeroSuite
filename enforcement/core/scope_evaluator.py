...
def is_historical_document_file(file_path: Path) -> bool:
    ...
    # Check for report files (e.g., MERGE_READINESS_REPORT.md, COMPLIANCE_REPORT.md)
    # These are historical documentation that should preserve their dates
    if '_report' in file_name or file_name.endswith('_report.md'):
        return True
    
    # Check for files in docs/reference/ directories (reference documentation)
    if 'docs/reference/' in str(file_path).replace('\\', '/').lower():
        return True
...
def is_log_file(file_path: Path) -> bool:
    ...
    # Check if file is in memory-bank directory (supports both .ai/memory_bank/ and .ai/memory-bank/)
    file_path_str = str(file_path).replace('\\', '/').lower()
    if "memory-bank" in file_path_str or "memory_bank" in file_path_str:
        return True
