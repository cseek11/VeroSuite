...
export default function MonthView({ events, onEdit, resources }) {
  ...
  return (
    <div className="month-view h-full w-full flex flex-col overflow-hidden">
      <div className="text-center text-lg font-semibold p-2 text-gray-800 border-b border-gray-200 bg-gray-50/50 flex-shrink-1">
        Month View
      </div>
      <div className="flex-1 overflow-auto p-1">
        <div className="grid grid-cols-7 gap-0.5 h-full">
          {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day, index) => (
            <div key={`day-${index}`} className="p-1 text-center text-xs font-medium bg-gray-100/50 text-gray-700 border-b border-gray-200">
              {day}
            </div>
          ))}
          {Array.from({length: 35}, (_, i) => (
            <div key={i} className="p-1 border border-gray-200 relative hover:bg-gray-50/50 flex flex-col min-h-0">
