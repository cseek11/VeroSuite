...
export default function DispatcherDashboard() {
  ...
  useEffect(() => {
    ...
    const mockJobs = [
      { id: '1', status: 'completed', scheduled_date: '2025-08-17', location: { name: 'Site A', coordinates: { lat: 40.44, lng: -79.99 } }, technician_id: 'tech1' },
      { id: '2', status: 'completed', scheduled_date: '2025-08-18', location: { name: 'Site B', coordinates: { lat: 40.45, lng: -79.98 } }, technician_id: 'tech2' },
      { id: '3', status: 'in_progress', scheduled_date: '2025-08-18', location: { name: 'Site C', coordinates: { lat: 40.46, lng: -79.97 } }, technician_id: 'tech1' },
      { id: '4', status: 'scheduled', scheduled_date: '2025-08-19', location: { name: 'Site D', coordinates: { lat: 40.47, lng: -79.96 } }, technician_id: null },
      { id: '5', status: 'unassigned', scheduled_date: '2025-08-19', location: { name: 'Site E', coordinates: { lat: 40.48, lng: -79.95 } }, technician_id: null },
    ];
    setJobs(mockJobs);
    setAccounts([]);
    setRoutes([]);
      ...
      {loading ? <div className="text-lg">Loading...</div> : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="space-y-4">
              {Array.isArray(jobs) && jobs.map((job) => (
                <div key={job.id} className="bg-white shadow rounded p-4 border-l-4 border-blue-400">
                  ...
                  <div className="text-xs text-gray-500">Technician: {job.technician_id || 'Unassigned'}</div>
                  <form className="mt-2 flex gap-2" onSubmit={handleAssignJob}>
                    <select value={assignTechId} onChange={e => setAssignTechId(e.target.value)} className="border rounded px-2">
                      <option value="">Select Technician</option>
                      {routes.map((route: any) => (
                        <option key={route.id} value={route.technician}>{route.technician}</option>
                      ))}
                    </select>
                    <button type="submit" className="px-2 py-1 bg-blue-500 text-white rounded" onClick={() => setAssignJobId(job.id)}>Assign</button>
                  </form>
                </div>
              ))}
              ...
              <>
                ...
                <div className="mt-8">
                  ...
                  <div className="space-y-2">
                    {auditLogs.length === 0 ? <div>No audit logs found.</div> : auditLogs.map((log: any) => (
                      <div key={log.id} className="bg-gray-100 rounded p-2 text-xs">
                        <div><span className="font-bold">{log.action}</span> on <span>{log.resource_type}</span> ({log.resource_id})</div>
          ...
          <div className="rounded shadow overflow-hidden">
            <MapContainer center={[40.4406, -79.9959]} zoom={12} style={{ height: '400px', width: '100%' }}>
              <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
              {Array.isArray(jobs) && jobs.map((job) => (
                job.location?.coordinates?.lat && job.location?.coordinates?.lng ? (
                  <Marker key={job.id} position={[job.location.coordinates.lat, job.location.coordinates.lng]}>
                    <Popup>
              ...
              {Array.isArray(routes) && routes.map((route: any, idx: number) => {
                ...
                const positions = routeJobs
                  .filter((job: any) => job && job.location?.coordinates?.lat && job.location?.coordinates?.lng)
                  .map((job: any) => [job.location.coordinates.lat, job.location.coordinates.lng]);
