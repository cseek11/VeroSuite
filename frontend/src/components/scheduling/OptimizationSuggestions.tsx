    ...
    queryFn: async () => {
      ...
      if (highUtilization.length > 0 && lowUtilization.length > 0) {
        suggestionsList.push({
          ...
          impact: `Redistributing ${highUtilization[0].jobCount} jobs from ${highUtilization[0].technicianName} could improve overall efficiency.`,
          actionItems: [
            `Move jobs from ${highUtilization[0].technicianName} (${highUtilization[0].utilizationRate.toFixed(1)}% utilization)`,
            `Assign jobs to ${lowUtilization[0].technicianName} (${lowUtilization[0].utilizationRate.toFixed(1)}% utilization)`,
