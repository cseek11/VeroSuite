...
// Generate nonce for CSP (should ideally come from server)
const generateNonce = (): string => {
  return `nonce-${Math.random().toString(36).substring(2, 15)}${Math.random().toString(36).substring(2, 15)}`;
};
...
export const WidgetSandbox: React.FC<WidgetSandboxProps> = ({
  ...
  // Generate nonce for CSP (should ideally come from server in production)
  const nonce = useRef<string>(generateNonce()).current;
  ...
  const handleMessage = useCallback((event: MessageEvent) => {
    ...
    try {
      ...
      if (event.origin !== widgetOrigin && event.origin !== window.location.origin) {
        console.warn('WidgetSandbox: Message from unauthorized origin', event.origin);
        return;
      }
    } catch (e) {
      console.warn('WidgetSandbox: Invalid widget entry point', manifest.entry_point);
      return;
    }
