    ...
    queryFn: async () => {
      try {
        ...
        if (enhancedApi.technicians && typeof enhancedApi.technicians.list === 'function') {
          ...
          users = await enhancedApi.users.list({ roles: ['technician'] });
        } else if (enhancedApi.users && typeof enhancedApi.users.getAll === 'function') {
          users = await enhancedApi.users.getAll();
          users = users.filter((u: any) => 
            u.roles?.includes('technician') || u.role === 'technician'
          );
        } else {
      ...
      if (technicianId) {
        ...
        if (job.scheduled_date && job.scheduled_start_time && job.scheduled_end_time && targetTechnician) {
          try {
            ...
            logger.warn('Failed to check availability, proceeding anyway', error);
          }
        }
      } else {
        ...
        if (job.scheduled_date && job.scheduled_start_time && job.scheduled_end_time) {
          try {
            ...
            const availableTechIds = available.filter((t: any) => t.is_available).map((t: any) => t.id);
            targetTechnician = techniciansData.find(t => availableTechIds.includes(t.id)) || techniciansData[0];
          } catch (error) {
            logger.warn('Failed to check availability, using fallback', error);
    ...
    <div className={`h-full flex flex-col min-h-[400px] ${className}`} data-card-id={cardId}>
            ...
            <div className="mt-6">
              ...
              <div className="space-y-2">
                {techniciansData.map((technician) => {
                  ...
                  const techAvailability = availableTechnicians?.find((t: any) => t.id === technician.id);
      ...
      {conflictData && (
        <ConflictResolutionDialog
          ...
          technicianName={pendingAssignment?.technician.name}
        />
      )}
    </div>
  );
