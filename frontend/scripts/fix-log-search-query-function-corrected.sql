-- Fix the log_search_query function to only use existing columns
...
-- Create the corrected function that only uses existing columns
...
BEGIN
    ...
    -- Insert the search log - only using columns that exist
...
-- Check the most recent search logs to see if query_text is now populated
SELECT 
    id,
    query_text,
    query_type,
