...
3. Tenant middleware sets the PostgreSQL `app.tenant_id` session variable using `SET LOCAL`, and sets the runtime role to `verofield_app`.
...
export class TenantMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      await this.database.query(`SET LOCAL ROLE verofield_app`);
    } catch (err) {
...
- [Security Architecture](../../architecture/security.md) - Security model overview
