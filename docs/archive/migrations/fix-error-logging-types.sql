-- ============================================================================
-- FIX ERROR LOGGING TYPE ISSUES
-- ============================================================================
-- Quick fix for type mismatch issues in error logging functions

-- Drop existing functions to recreate with proper types
DROP FUNCTION IF EXISTS log_search_success(TEXT, TEXT, INTEGER, INTEGER, JSONB);
...
-- Recreate log_search_success with proper type handling
...
-- Recreate get_error_statistics with proper type handling
...
-- Recreate get_recent_errors with proper type handling
...
-- Recreate get_search_performance_metrics with proper type handling
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(jsonb_object_agg(search_type, type_count), '{}'::jsonb) as searches_by_type,
    COALESCE(jsonb_object_agg(
      date_trunc('day', created_at)::date::text,
      daily_count
    ), '{}'::jsonb) as performance_trend
  FROM (
    SELECT 
      search_type,
      COUNT(*) as type_count
...
-- Recreate resolve_error with proper type handling
...
BEGIN
    ...
    RAISE NOTICE '🎉 Error logging system type fixes completed successfully!';
END $$;
