...
3.13.x CPython 3.13 Experimental JIT (Copy-and-Patch)

Python 3.13 introduces an optional, experimental JIT compiler enabled at build time (--enable-experimental-jit). The implementation is a copy-and-patch JIT (PEP 744), not LLVM-based.

CPython still uses the regular bytecode interpreter as tier 0.

"Hot" regions of bytecode are compiled by stitching together pre-generated machine code templates and patching constants, jump targets, and metadata at runtime.
...
In current benchmarks, typical end-to-end speedups are in the 5–15% range on pyperformance, with larger gains on tight numeric / control-flow heavy code and negligible benefits for I/O-bound or extension-heavy workloads.
...
Not production-ready (as of 2025), but:
...
4.5.7.1 Variadic Generics: TypeVarTuple & Unpack (PEP 646)

Motivation: typing *args, NamedTuple, heterogeneous tuples, and array-like shapes.

TypeVarTuple and Unpack patterns:
...
class Array[*Shape]:
    ...
    def reshape[*NewShape](self) -> Array[*NewShape]: ...

# Usage
arr: Array[10, 20, 30]  # 3D array
```

4.5.7.2 PEP 695 Generic Type Syntax (Python 3.12+)
...
Scoping: type parameters are lexically scoped to the function/class/type alias.

Tooling caveats: older type checkers may not fully support PEP 695 yet.

Migration advice: how to migrate from TypeVar-heavy legacy code:
...
5.4.9 Structural Pattern Matching — Advanced Cases

OR (|) Patterns

Semantics of case A | B:; exhaustive vs overlapping patterns

"Decision-tree" compilation and backtracking behavior
...
AS (as) Patterns for Capture

```python
match node:
    case BinaryOp(left, op, right) as expr if op == "+":
        optimize_add(expr)
```

Using guards correctly, avoiding side-effectful guards:

⚠️ Guards should be pure functions; side effects in guards lead to unpredictable behavior.

Class Patterns and __match_args__

How Python maps positional patterns to attributes via __match_args__

Designing domain objects with match-friendly APIs (immutable, minimal):
...
class Point:
    x: float
    y: float
    __match_args__ = ("x", "y")  # Explicit positional matching
...
Mapping vs Sequence Patterns

Precise rules: mapping patterns use keys, sequences use index positions

Pitfalls with dict subclasses and order-dependent logic:

⚠️ Dict patterns match by key, not insertion order (though CPython 3.7+ preserves order, pattern matching doesn't guarantee it).

Performance & Readability Guidelines

When pattern matching improves clarity:

Complex nested data structures

Multiple related conditions on same value

When plain if/elif chains are better (e.g. heavily stateful branches):

Simple boolean conditions

Stateful operations that modify context
...
User.greet(u)  # Python inserts self automatically
...
8.2.1 Import Machinery Internals (importlib.machinery)
...
High-level pipeline: sys.meta_path → finders → ModuleSpec → loader → execution.

ModuleSpec: what it contains (name, loader, origin, submodule_search_locations).

Common loaders: SourceFileLoader, ExtensionFileLoader, NamespaceLoader.

Dynamic imports using importlib.import_module, spec_from_file_location, and custom finders:
...
Manipulating __path__ and namespace packages; when to reach for them (plugin systems, multi-repo layouts):

```python
# Custom finder example
class CustomFinder:
    def find_spec(self, name, path, target=None):
        if name.startswith("myplugin."):
...
10.13.3 Exception Groups & except* (Python 3.11+) — Deep Dive

Concept: ExceptionGroup aggregates multiple exceptions, typically from concurrent tasks.

Syntax: except* handles matching exceptions inside a group without swallowing the others.

Nesting: how nested ExceptionGroups are normalized and flattened.

Real-World Usage:

asyncio.TaskGroup / anyio / Trio style concurrency
...
async def process_batch(items):
    ...
    # If any task fails, ExceptionGroup is raised
```

concurrent.futures batch job failures
...
Use ExceptionGroup for "many peers failed, continue processing".

Log full groups; design error payloads to be structured, not just strings.

Selective handling example:

```python
try:
    run_parallel_tasks()
except* ValueError as eg:
    # Handle only ValueError exceptions, others propagate
    for exc in eg.exceptions:
        logger.warning(f"Value error: {exc}")
except* TypeError:
    # Handle TypeError separately
    pass
# Other exceptions in group still propagate
```
...
11.14.1 Distributed Tracing & Context Propagation

OpenTelemetry basics: traces, spans, and context propagation across services.

How to propagate trace IDs through HTTP headers, message queues, background jobs:
...
Example: instrumenting a FastAPI + Celery stack with trace continuity end-to-end.

11.14.2 Log Aggregation & Metrics Cardinality

Structured logging with correlation IDs, trace IDs, and user / tenant tags.

High-cardinality metric pitfalls and strategies (label hygiene, sketches, aggregation).

SLO / SLI definitions for Python services:

SLI examples: request latency, error rate, data freshness, queue depth.

SLO templates: "99.9% of write requests < 200 ms over 30 days".
...
12.6.1 CPython GC: Reference Counting + Cyclic Collector

Primary mechanism: reference counting; objects are deallocated when refcount hits zero.

Generational cyclic GC: detects and frees cycles that refcount alone can't handle.

Tri-color marking model: white/grey/black sets and the tri-color invariant.

Tunable thresholds, and when it makes sense to adjust them for long-lived services:
...
12.6.2 Frame Object Optimizations (3.11+)

Streamlined frame structure and reuse of frame space on the C stack.

"Inlined" Python function calls → fewer C calls, better cache locality.

Debugging trade-offs (lazy frames vs full introspection):

```python
import sys
sys._getframe()  # May return None in optimized builds
```
...
For large, numeric workloads, vectorized NumPy operations are typically 10–100× faster than equivalent pure-Python loops, because the heavy lifting happens in optimized C code and uses contiguous, typed memory. On very small arrays the overhead of NumPy can actually make pure Python faster.
...
12.9.3.1 The Buffer Protocol & Zero-Copy Interop

Definition: the buffer protocol (PEP 3118, formalized at Python level by PEP 688) lets objects expose raw memory via Py_buffer / memoryview.

Key Players: bytes, bytearray, memoryview, NumPy arrays, array module, many C extensions.

Use Cases:

Zero-copy slicing and views across libraries:

```python
import numpy as np
arr = np.arange(100)
view = memoryview(arr)  # Zero-copy view
slice_view = view[10:20]  # Still zero-copy
```

Writing custom C extensions / Cython code that interoperate with NumPy:

```python
# Cython example
cdef void process_buffer(double[:] arr):
...
Avoid copying when passing large arrays across layers; prefer memoryview.

Clearly document mutability expectations (read-only vs writable buffers):

```python
# Read-only view
view = memoryview(arr).tobytes()  # Forces copy if needed

# Writable view
writable = memoryview(bytearray(b"data"))
```
...
13.10.1 WASM / Pyodide

Pyodide: CPython compiled to WebAssembly, with key scientific stack ported.

Design patterns for "Python in the browser": client-side notebooks, teaching tools, offline UIs:

```python
# Pyodide example - loading in browser
import pyodide_js
await pyodide_js.loadPackage("numpy")
import numpy as np
arr = np.array([1, 2, 3])
print(arr.sum())  # Works in browser!

# For notebooks
from pyodide import open_url
code = open_url("https://example.com/analysis.py").read()
exec(code)
```

Constraints: cold-start latency, limited threading, filesystem sandboxing:

⚠️ Cold-start latency: First load can take 2-5 seconds to download and initialize Pyodide runtime.

⚠️ Limited threading: WebAssembly doesn't support true threading; use Web Workers for parallelism.

⚠️ Filesystem sandboxing: File operations are virtualized; use IndexedDB or fetch() for persistence.
...
15.3.2.1 Ruff as a Unified Lint/Format Tool

Linter + formatter + import sorter, written in Rust.

Can replace Flake8 (+ plugins), isort, Black, etc., with 10–100× speedups.

Recommended "enterprise preset": what rules to enable, and how to integrate via pre-commit + CI:
...
15.6.0 Secure Container Images

Multi-stage Docker builds; stripping dev tools from runtime image.

Distroless / minimal images and their impact on attack surface:
...
Non-root users, read-only root filesystems, dropping Linux capabilities:
...
15.6.0.1 Image Scanning & SBOMs

trivy for vulnerability scanning and SBOM generation + scanning CycloneDX/SPDX SBOMs.

grype + syft workflow: generate SBOM with Syft, scan with Grype.

CI integration patterns: fail on critical CVEs, allowlist, and remediation workflows:
...
Threads	❌ (≤3.12) / ✅ (3.13 FT)	Network IO, HTTP clients, websockets	CPU-bound work
Free-Threaded CPython 3.13+	✅ (experimental)	CPU-bound parallel workloads	Single-threaded (may slow down)
...
Introduced in 3.13 as a separate build with the GIL disabled (e.g. python3.13t, or --disable-gil when building from source).
...
⚠️ Reality Check: Free-Threading in Production
...
Descriptor Protocol Resolution Order:

```
Attribute Lookup: obj.attr
┌─────────────────────────────────────────┐
│ 1. Check instance.__dict__['attr']     │  ← Direct instance attribute
└─────────────────────────────────────────┘
         ↓ (not found)
┌─────────────────────────────────────────┐
│ 2. Check type(obj).__dict__['attr']     │  ← Class attribute
└─────────────────────────────────────────┘
         ↓ (found descriptor)
┌─────────────────────────────────────────┐
│ 3. Check if descriptor:                  │
│    - Data descriptor (has __set__)       │  ← Overrides instance
│    - Non-data descriptor (only __get__)  │  ← Falls back to instance
└─────────────────────────────────────────┘
         ↓
┌─────────────────────────────────────────┐
│ 4. Call descriptor.__get__(obj, type)    │
└─────────────────────────────────────────┘
```
...
20.2.1 Polars Deep Dive

Rust-backed, columnar, multi-threaded DataFrame engine; supports eager & lazy execution and Arrow-based memory.

When to choose Polars over pandas: large, wide, analytical workloads; streaming; >RAM datasets.

Interop with Arrow, Parquet, and cloud-native data lakes:
...
21.4.4.1 uv: Next-Gen Package & Project Manager

Rust-based, extremely fast resolver and installer; drop-in for pip/pip-tools workflows.

Handles virtualenv management, lockfiles, and project scaffolding.

Guidance: when to standardize on uv for new repos vs legacy pip/pipenv/poetry:
...
23.1.1 12-Factor Apps for Python

Brief overview of the 12 factors and how they map to Django/FastAPI/Celery apps: config in env, stateless processes, backing services, logs as event streams.

Key factors for Python:

I. Codebase: One codebase tracked in revision control, many deploys

II. Dependencies: Explicitly declare and isolate dependencies (requirements.txt, pyproject.toml)

III. Config: Store config in the environment (os.getenv, pydantic-settings)

IV. Backing services: Treat backing services as attached resources (database URLs, Redis URLs)

V. Build, release, run: Strictly separate build and run stages

VI. Processes: Execute the app as one or more stateless processes

VII. Port binding: Export services via port binding (uvicorn --port 8000)

VIII. Concurrency: Scale out via the process model (gunicorn workers, kubernetes pods)

IX. Disposability: Maximize robustness with fast startup and graceful shutdown

X. Dev/prod parity: Keep development, staging, and production as similar as possible

XI. Logs: Treat logs as event streams (structured logging, stdout/stderr)

XII. Admin processes: Run admin/management tasks as one-off processes (Django manage.py, Alembic migrations)
...
25.11.1 Service Mesh Integration

Role of Istio / Linkerd as an infrastructure layer for security, observability, and traffic management—no app changes required.

How Python services should expose metrics, health endpoints, and tracing headers to take full advantage:

```python
# FastAPI health endpoint for service mesh
@app.get("/health")
async def health():
...
25.11.2 Operators & Controllers

Python-based Kubernetes operators (e.g. kopf) for automating domain logic: schema migrations, scheduled jobs, state reconciliation.

Patterns for storing state: configmaps vs CRDs vs managed databases:

```python
import kopf

@kopf.on.create('myapp.example.com', 'v1', 'myresources')
...
26.16.1 Operational Semantics for a Python Core Fragment

Define a minimal lambda-calculus + mutable store fragment (expressions, statements, exceptions).

Small-step rules for evaluation, with an environment and store:

```
Evaluation Context:
E ::= [] | E op e | v op E | f(E) | if E then e1 else e2

Small-Step Rules:
⟨x, σ, ρ⟩ → ⟨ρ(x), σ, ρ⟩                    (Variable lookup)
⟨v1 + v2, σ, ρ⟩ → ⟨v1 + v2, σ, ρ⟩           (Addition, if v1, v2 are numbers)
⟨if True then e1 else e2, σ, ρ⟩ → ⟨e1, σ, ρ⟩ (Conditional true)
⟨if False then e1 else e2, σ, ρ⟩ → ⟨e2, σ, ρ⟩ (Conditional false)
```

26.16.2 Abstract Interpretation

Define abstract domains for "possible types" and "possible states".

Show how static analyzers (like mypy or security tools) can be framed as abstract interpreters:
...
Abstract Semantics:
⟦x + y⟧# = if ⟦x⟧# = int ∧ ⟦y⟧# = int then int else ⊤
```

Link to CPython Optimizations:

Show how constant folding and dead-code elimination arise naturally from the semantics:

```
Constant Folding:
If ⟦e⟧# = c (constant), then replace e with c at compile time.

Dead Code Elimination:
If ⟦guard⟧# = False, then eliminate the guarded branch.
```
...
27.4.1 pymalloc & Small-Object Allocation

Arenas (256 KiB / 1 MiB), pools (4 KiB), blocks (size classes ≤512 bytes).

Fragmentation behavior and why long-lived objects can cause memory bloat:

Small objects (≤512 bytes) use pymalloc arenas for fast allocation. Large objects bypass pymalloc and go directly to system malloc. Long-lived small objects can fragment arena pools, leading to memory bloat even when objects are freed.

27.5 PyObject / PyLongObject / PyListObject

PyObject header (refcount + type pointer) as the base for all objects.
...
PyObject Structure:
┌─────────────────┐
│ ob_refcnt       │  Reference count (4-8 bytes)
│ ob_type         │  Pointer to type object
└─────────────────┘
       ↓
┌─────────────────┐
│ Type-specific   │  Additional fields per type
│ data fields     │
└─────────────────┘

PyListObject Layout:
┌─────────────────┐
│ PyObject_VAR_HEAD│  Base + size
│ ob_item         │  Array of PyObject* pointers
│ allocated       │  Total allocated slots
└─────────────────┘
```

PyLongObject: variable-length "digits", sign encoded in ob_size.

PyListObject: over-allocated dynamic array of pointers; growth strategy & complexity guarantees:
...
Growth strategy: roughly 1.125× expansion, amortized O(1) append.

27.5.1 Dict Internals Memory Layout
...
Dict Structure:
┌─────────────────┐
│ PyObject_HEAD   │  Base object header
│ ma_used         │  Number of active entries
│ ma_version_tag  │  Version for safe iteration
│ ma_keys         │  Keys array (indices)
│ ma_values       │  Values array (or NULL)
└─────────────────┘

Hash Table Layout:
┌──────┬──────┬──────┐
│ idx  │ hash │ key  │  Entry slots
├──────┼──────┼──────┤
│  0   │ 123  │ "a"  │
│  1   │ 456  │ "b"  │
│  2   │ DUMMY│      │  Deleted entry marker
│  3   │ 789  │ "c"  │
└──────┴──────┴──────┘
```

27.5.1 Unicode Internals (PEP 393)

Flexible string representation: 1/2/4-byte storage per codepoint based on max ordinal.

PyASCIIObject vs PyCompactUnicodeObject.

Impact on C extensions and encoding/decoding speed:

ASCII strings use 1-byte storage; strings with higher codepoints use 2 or 4 bytes per character. This minimizes memory for ASCII-heavy workloads while supporting full Unicode.

27.5.2 Constant Folding & Peephole Optimizations

Compiler and peephole optimizer roles (AST-level vs bytecode-level).

Examples: literal folding (2+2 → 4), dead jump removal, constant tuple/list building.

How this interacts with the adaptive interpreter and JIT:

Constant folding happens at compile time (AST → bytecode). Peephole optimizations happen on bytecode. The adaptive interpreter and JIT can further optimize hot paths at runtime.
...
Real-world benchmarks for the 3.13 experimental JIT typically show single-digit to mid-teens percent speedups (~5–15%) on the standard pyperformance suite. Certain micro-benchmarks and hot loops can see much larger gains; I/O-bound and extension-heavy workloads often see little change.

27.16.1 Benchmark Design for JIT Evaluation

Use pyperformance as a baseline.

Add your own domain-specific micro-benchmarks (e.g. web endpoints, data transformations).

Measure: p50 / p95 latency, CPU utilization, memory overhead.
...
🧠 Chapter 29 — Modern Python Development Workflows (AI-Assisted)
AI-Assisted Development, Multi-Agent Systems, LLM Engineering & Code Quality Enforcement

29.1 — Introduction

AI-assisted development is transforming how Python code is written, reviewed, and maintained. This chapter covers:

AI-assisted code generation (Copilot, ChatGPT, Cursor)

Patterns for "AI in the loop": code review, refactoring, docs generation

Guardrails: linters, type checkers, policy engines to keep AI-generated code safe

Template prompts for common workflows (migration, performance tuning)

29.1.1 — AI Agents in Python Development
...
29.10 — AI-Assisted Code Generation Patterns

29.10.1 — Code Generation (Copilot, ChatGPT, Cursor)

Best practices for prompting:

Provide full context (file paths, existing patterns, constraints)

Request step-by-step reasoning before code

Use Python-styled prompts (follow PEP 8, type hints, docstrings)

Always validate with linters and type checkers

Example prompt template:

```
Refactor this function to use async/await:
- File: apps/api/src/users/user_service.py
- Current pattern: Uses ThreadPoolExecutor
- Constraint: Must maintain backward compatibility
- Style: Follow existing codebase patterns (see apps/api/src/orders/)
- Type hints: Required
- Tests: Update existing tests in tests/unit/users/
```

29.10.2 — Patterns for "AI in the Loop"

Code review: Use AI to suggest improvements, but human review is mandatory

Refactoring: Generate refactored code, then verify with tests and manual inspection

Docs generation: AI can generate docstrings, but verify accuracy and completeness

29.10.3 — Guardrails for AI-Generated Code

Linters: Always run ruff, mypy, pylint on AI-generated code

Type checkers: Verify type annotations are correct and complete

Policy engines: Use tools like Semgrep to enforce security and architecture rules

CI integration: Never merge AI-generated code without passing all CI checks

29.10.4 — Template Prompts for Common Workflows

Migration prompts:

```
Migrate this Flask route to FastAPI:
...
- Tools available: [NumPy, Cython, etc.]
```

29.11 — Case Studies

29.11.1 — End-to-End Microservice Example
...
API service, worker service, database, message broker, metrics/trace exporter.

Circuit-breaker / retries / idempotency patterns:
...
async def call_external_api(data: dict) -> dict:
    ...
    headers = {"Idempotency-Key": generate_key(data)}
    response = await httpx.post(url, json=data, headers=headers)
    return response.json()
```

29.11.2 — CI/CD Pipelines

Full GitHub Actions example with matrix (3.10–3.13), ruff, mypy, tests, Docker build+scan, deploy:
...
29.11.3 — Production Debugging Playbook

"Symptom → Observability → Hypothesis → Instrumentation → Fix" workflow:

1. Symptom: High memory usage, slow endpoints, CPU spikes

2. Observability: Check logs, metrics, traces
...
3. Hypothesis: Memory leak in pymalloc, arenas/pools/blocks

4. Instrumentation: Add profiling, increase logging verbosity

5. Fix: Identify root cause, implement fix, verify

Memory leaks (pymalloc, arenas/pools/blocks):
...
# ... operation ...
after = check_memory()
print(f"Memory delta: {after - before} bytes")
```

CPU spikes, slow endpoints (profilers, trace sampling, pydumps):
...
29.11.4 — Migration Guides

Flask → FastAPI: routing, dependency injection, async, typing:
...
sync → async (web handlers, DB drivers, background tasks):
...
async def process_order(order_id: int):
    async with async_db.session() as session:
        ...
        await send_email_async(order.customer_email)
        return order
```

Python 2 → 3 as a historical case study in large-scale migration:
...
Tooling: 2to3, modernize, futurize
...
See Chapter 4.7 for full treatment of ABCs and their use cases.
...
Python's default sorting algorithm for lists. A stable, hybrid sorting algorithm derived from merge sort and insertion sort. It is adaptive: it detects existing ordered "runs" in the input and optimizes for partially sorted data, which makes it particularly efficient on real-world workloads (often close to O(n) when data has structure).

Why Python chose it:

Good worst-case guarantees (O(n log n))

Excellent performance on partially ordered and real application data

Stable, which matters for multi-key / chained sorts
