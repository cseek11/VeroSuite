...
- POST /v1/billing/invoices/:id/stripe-payment-intent
  - Creates a Stripe payment intent for an invoice
  - Returns: { clientSecret, paymentIntentId, amount, currency, status }
  - Requires: Invoice must not be already paid
  - Minimum amount: $0.50

- GET /v1/billing/stripe/payment-status/:paymentIntentId
  - Gets the current status of a Stripe payment intent
  - Returns: { id, status, amount, currency, created, metadata }
...
- POST /v1/billing/stripe/webhook
  ...
  - Note: This endpoint bypasses JWT authentication (webhook-only)
