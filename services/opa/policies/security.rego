...
deny contains msg if {
    ...
    table_name := table_matches[0][1] if count(table_matches) > 0
    table_name := "unknown" if count(table_matches) == 0
...
deny contains msg if {
    ...
    not has_override_marker(input.pr_body, "security-logging")
...
deny contains msg if {
    ...
    not has_override_marker(input.pr_body, "input-validation")
