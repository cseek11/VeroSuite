...
# Collect all violations (R08 + R09)
violations := array.concat(
    array.concat(
        array.concat(
            array.concat(console_log_violations, unstructured_logging_violations),
            array.concat(missing_required_fields_violations, missing_trace_id_violations)
        ),
        missing_context_operation_violations
    ),
    array.concat(
        array.concat(
            array.concat(missing_http_trace_violations, missing_service_trace_violations),
            missing_db_trace_violations
        ),
        array.concat(missing_external_trace_violations, missing_mq_trace_violations)
    )
)

# Collect all warnings (R08 + R09)
warnings := array.concat(incomplete_logging_warnings, incomplete_trace_propagation_warnings)
