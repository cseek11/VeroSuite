...
missing_unit_tests_violations[msg] if {
    some file in input.files
    ...
    not has_test_file(file.path, input.files)
...
missing_unit_tests_violations[msg] if {
    ...
    not file_was_modified(test_file_path, input.files)
...
coverage_below_threshold_violations[msg] if {
    some file in input.coverage_report.files
    is_new_or_modified_file(file.path, input.files)
...
tests_skipped_violations[msg] if {
    some file in input.files
...
coverage_delta_negative_violations[msg] if {
...
incomplete_test_coverage_warnings[msg] if {
    some file in input.coverage_report.files
    is_new_or_modified_file(file.path, input.files)
...
get_test_file_path(source_path) := test_path if {
    ...
    test_path := concat("/", [dir, "__tests__", test_basename])
}
...
additional_testing_warnings[msg] if {
    some file in input.files
    needs_error_path_tests(file)
    
    test_file_path := get_test_file_path(file.path)
    not has_error_path_tests(test_file_path, input.files)
...
additional_testing_warnings[msg] if {
    ...
    test_file_path := get_test_file_path(file.path)
    not has_state_machine_tests(test_file_path, input.files)
...
additional_testing_warnings[msg] if {
    ...
    test_file_path := get_test_file_path(file.path)
    not has_tenant_isolation_tests(test_file_path, input.files)
...
additional_testing_warnings[msg] if {
    ...
    test_file_path := get_test_file_path(file.path)
    not has_observability_tests(test_file_path, input.files)
...
additional_testing_warnings[msg] if {
    ...
    test_file_path := get_test_file_path(file.path)
    not has_security_tests(test_file_path, input.files)
...
additional_testing_warnings[msg] if {
    ...
    test_file_path := get_test_file_path(file.path)
    not has_data_migration_tests(test_file_path, input.files)
...
additional_testing_warnings[msg] if {
    ...
    test_file_path := get_test_file_path(file.path)
    not has_performance_tests(test_file_path, input.files)
...
additional_testing_warnings[msg] if {
    ...
    test_file_path := get_test_file_path(file.path)
    not has_accessibility_tests(test_file_path, input.files)
...
# Helper: Check if test file has error path tests
has_error_path_tests(test_file_path, files) if {
    some file in files
    file.path == test_file_path
    regex.match(`throw.*Exception|expect.*toThrow|should.*error`, file.content)
}

# Helper: Check if test file has state machine tests
has_state_machine_tests(test_file_path, files) if {
    some file in files
    file.path == test_file_path
    regex.match(`transition.*status|legal.*transition|illegal.*transition`, file.content)
}

# Helper: Check if test file has tenant isolation tests
has_tenant_isolation_tests(test_file_path, files) if {
    some file in files
    file.path == test_file_path
    regex.match(`tenant.*isolation|cross.*tenant|RLS.*policy`, file.content)
}

# Helper: Check if test file has observability tests
has_observability_tests(test_file_path, files) if {
    some file in files
    file.path == test_file_path
    regex.match(`structured.*log|traceId|trace.*propagation`, file.content)
}

# Helper: Check if test file has security tests
has_security_tests(test_file_path, files) if {
    some file in files
    file.path == test_file_path
    regex.match(`authentication|authorization|input.*validation|security`, file.content)
}

# Helper: Check if test file has data migration tests
has_data_migration_tests(test_file_path, files) if {
    some file in files
    file.path == test_file_path
    regex.match(`migration.*idempotency|data.*integrity|rollback`, file.content)
}

# Helper: Check if test file has performance tests
has_performance_tests(test_file_path, files) if {
    some file in files
    file.path == test_file_path
    regex.match(`performance|response.*time|latency|p95|p99`, file.content)
}

# Helper: Check if test file has accessibility tests
has_accessibility_tests(test_file_path, files) if {
    some file in files
    file.path == test_file_path
...
coverage_degraded(file, baseline_coverage, current_coverage) if {
...
coverage_requirements_warnings[msg] if {
    some file in input.files
    is_source_file(file.path)
...
performance_degraded(endpoint, baseline_p50, current_p50) if {
