...
# This checks if TODO is removed (line starting with -) AND implementation added (line starting with +)
appears_resolved(file) if {
	# Check if diff shows TODO removal (lines starting with -)
	regex.match("-.*TODO:", file.diff)

	# Also check that implementation was added (has + lines after TODO removal)
...
appears_resolved(file) if {
	...
	# Also check that implementation was added (has + lines after FIXME removal)
	regex.match("\\+.*", file.diff)
}
...
# This rule checks for TODOs that are still present (not removed) but work appears complete
# If TODO is removed (appears_resolved), we don't warn - that's the correct behavior
# This rule should NOT trigger for vague TODOs (handled by R15-W05) or meaningful TODOs without clear implementation
warn contains msg if {
	input_valid
	some file in input.changed_files
	not is_tech_debt_file(file)
	...
	# Check if TODO is still present (not removed) - look for + lines with TODO
	regex.match("\\+.*TODO:", file.diff)

	# But implementation was also added (work appears complete) - check for actual code, not just "function"
	regex.match("\\+.*(function|class|const|let|var|return|if|for|while)", file.diff)

	# TODO was NOT removed (no - line with TODO)
	not regex.match("-.*TODO:", file.diff)

	# TODO has clear action (not vague) - exclude vague TODOs handled by R15-W05
	not regex.match("TODO:\\s*(\\n|$)", file.diff)

	# TODO is not meaningful (no workaround/deferred/temporary keywords) - meaningful TODOs handled by R15-W02/W03
	not has_meaningful_todo_keywords(file)
...
warn contains msg if {
	...
	# Match "TODO:" followed by optional whitespace and then newline or end of string
	# Pattern: TODO: followed by optional whitespace, then newline or end of string
	regex.match("TODO:\\s*(\\n|$)", file.diff)
