...
missing_transaction[msg] if {
	...
	method != null
	not has_transaction_wrapper(file.content)
...
# Guard: Ensure input.changed_files exists before processing
input_valid if {
	is_array(input.changed_files)
}

# ============================================================================
# DRY Helper: Aggregate violation messages
# ============================================================================

# Helper function to aggregate violation messages from a partial rule set
...
deny[msg] if {
	...
	violations := aggregate_violations(business_logic_in_controller)
...
deny[msg] if {
	...
	violations := aggregate_violations(missing_dto_validation)
...
deny[msg] if {
	...
	violations := aggregate_violations(missing_tenant_isolation)
...
deny[msg] if {
	...
	violations := aggregate_violations(missing_transaction)
...
deny[msg] if {
	...
	violations := aggregate_violations(service_passthrough)
...
warn[msg] if {
	...
	warnings := aggregate_violations(incomplete_patterns)
...
has_unvalidated_properties(content) if {
	...
	# NOTE: Intentional strictness - This check requires @IsString() to be present
	# in the file if any string properties exist. DTOs using only @IsEmail(),
	# @IsUUID(), or other validators (without @IsString()) will be flagged.
	# This enforces a style where string properties must have @IsString() plus
	# additional validators. If your style allows other validators without @IsString(),
...
find_prisma_query_without_tenant(content) := query if {
	...
	# File-level heuristic: any tenant_id anywhere defeats this check
...
has_transaction_wrapper(content) if {
	contains(content, "$transaction")
}
...
# Helper: Build set of missing patterns
...
pattern_present("error_handling", content) if error_handling_present(content)
pattern_present("audit_logging", content) if audit_logging_present(content)
pattern_present("structured_logging", content) if structured_logging_present(content)

find_missing_patterns(content) := patterns if {
	missing_set := _missing_patterns_set(content)
	# Convert set to array
	patterns := [name | name := missing_set[_]]
}

error_handling_present(content) if contains(content, "try")
error_handling_present(content) if contains(content, "catch")

audit_logging_present(content) if contains(content, "audit.log")
audit_logging_present(content) if contains(content, "auditService")

structured_logging_present(content) if contains(content, "logger.")
structured_logging_present(content) if contains(content, "Logger")
