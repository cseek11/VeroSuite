...
test_missing_validation_decorators {
    mock_input := {
        "changed_files": [
            {
                "path": "apps/api/src/user/dto/create-user.dto.ts",
...
test_file_upload_validation_present {
    mock_input := {
        "changed_files": [
            {
...
test_missing_file_upload_validation {
    mock_input := {
        "changed_files": [
            {
                "path": "apps/api/src/upload/upload.controller.ts",
...
test_sql_parameterization_present {
    mock_input := {
        "changed_files": [
            {
                "path": "apps/api/src/user/user.service.ts",
...
test_config_sanitization_warning {
    mock_input := {
        "changed_files": [
            {
...
test_validation_message_warning {
    mock_input := {
        "changed_files": [
            {
                "path": "apps/api/src/user/dto/create-user.dto.ts",
    ...
    warning := [msg | msg := warn[_]; contains(msg, "custom error messages")][0]
    contains(warning, "WARNING [Security/R13]")
}
...
test_html_content_without_sanitization {
    mock_input := {
        "changed_files": [
            {
    ...
    violation := [msg | msg := deny[_]; contains(msg, "sanitization")][0]
    contains(violation, "stored without sanitization")
}

# Test 19: dangerouslySetInnerHTML without sanitization (violation)
test_dangerous_inner_html_without_sanitization {
    mock_input := {
        "changed_files": [
            {
    ...
    violation := [msg | msg := deny[_]; contains(msg, "dangerouslySetInnerHTML")][0]
    contains(violation, "without sanitization")
}

# Test 20: Non-validation file (no violation)
test_non_validation_file {
    mock_input := {
        "changed_files": [
            {
