...
test_deny_on_forbidden_pattern if {
    ...
    count(deny) with input as test_input == 1
}
...
test_override_required_on_critical_file if {
    ...
    count(override) with input as test_input == 1
}
...
test_override_marker_allows_critical_change if {
    ...
    count(override) with input as test_input == 0
}
...
test_warning_on_empty_pr_body if {
    ...
    count(warn) with input as test_input == 1
}
