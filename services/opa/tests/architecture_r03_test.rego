...
test_correct_path_passes if {
    ...
    count(architecture.deny) == 0 with input as test_input
}
...
test_new_microservice_fails if {
    ...
    count(architecture.deny) > 0 with input as test_input
    some msg in architecture.deny with input as test_input
    contains(msg, "R03")
...
test_utility_in_service_warns if {
    ...
    count(architecture.warn) > 0 with input as test_input
    some msg in architecture.warn with input as test_input
    contains(msg, "R03")
...
test_violation_with_override_passes if {
    test_input := {
        ...
        "pr_body": "Add admin service\n\n@override:architecture-boundaries\nJustification: Admin service required for system administration. Approved by architecture team on 2025-11-30. See docs/architecture/decisions/003-admin-service.md"
    }
    count(architecture.deny) == 0 with input as test_input
}
...
test_multiple_violations_detected if {
    ...
    count(architecture.deny) >= 1 with input as test_input
}
