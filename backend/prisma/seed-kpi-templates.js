...
async function main() {
    ...
    const { kpiTemplates: templateDefinitions } = await Promise.resolve().then(() => require('./kpi-templates-clean'));
    console.log(`📊 Seeding ${templateDefinitions.length} KPI Templates...`);
    const kpiTemplates = await Promise.all(templateDefinitions.map(template => prisma.kpiTemplate.create({
        data: {
            name: template.name,
