...
async function main() {
    ...
    const { kpiTemplates: templateDefinitions } = await Promise.resolve().then(() => require('./kpi-templates-seed'));
    const kpiTemplates = await Promise.all(templateDefinitions.map(template => prisma.kpiTemplate.upsert({
        where: { id: template.id },
        update: {},
        create: Object.assign(Object.assign({}, template), { tenant_id: tenantId, formula_fields: template.formula_fields, status: 'published' })
    })));
