...
describe('CRM Workflow Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        await app.init();
        testDb = enterprise_setup_1.TestDatabase.getInstance();
        await testDb.setup();
    ...
    describe('Complete Customer Lifecycle', () => {
        it('should handle complete customer lifecycle from lead to completion', async () => {
            ...
            const workOrderData = enterprise_setup_1.MockFactory.createWorkOrder({
    ...
    describe('Multi-Tenant Isolation', () => {
        it('should enforce tenant isolation across all operations', async () => {
            const tenant1Customer = enterprise_setup_1.MockFactory.createCustomer({
                tenant_id: 'tenant-1',
            ...
            const tenant2Token = 'tenant-2-auth-token';
    ...
    describe('Data Consistency and Integrity', () => {
        it('should maintain referential integrity across related entities', async () => {
            ...
            const workOrderResponse = await request(app.getHttpServer())
                ...
                .expect(201);
            expect(workOrderResponse.body.customer_id).toBe(customerId);
            const invalidWorkOrderData = enterprise_setup_1.MockFactory.createWorkOrder({
    ...
    describe('Performance Under Load', () => {
        ...
        it('should handle large dataset queries efficiently', async () => {
            ...
            for (let i = 0; i < 50; i++) {
                ...
                customerPromises.push(request(app.getHttpServer())
                    .post('/api/customers')
                    .set('Authorization', `Bearer ${authToken}`)
                    .send(customerData));
            }
    ...
    describe('Business Rule Validation', () => {
        it('should enforce business rules for work order scheduling', async () => {
            ...
            const customerResponse = await request(app.getHttpServer())
                ...
                .expect(201);
            const customerId = customerResponse.body.id;
            const pastWorkOrderData = enterprise_setup_1.MockFactory.createWorkOrder({
                customer_id: customerId,
            ...
            await request(app.getHttpServer())
                ...
                .expect(400);
            const futureWorkOrderData = enterprise_setup_1.MockFactory.createWorkOrder({
                customer_id: customerId,
                scheduled_date: new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString()
            });
            await request(app.getHttpServer())
...
//# sourceMappingURL=crm-workflow.test.js.map