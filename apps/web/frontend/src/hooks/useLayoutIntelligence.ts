...
export function useLayoutIntelligence({
  ...
  const optimizeLayout = useCallback((): DashboardRegion[] => {
    ...
    optimized.sort((a, b) => {
      ...
      if (isPeakHours) {
        if (a.region_type === RegionType.SCHEDULING) return -1;
        if (b.region_type === RegionType.SCHEDULING) return 1;
      }

      // Priority 2: Reports/analytics during low workload
      if (!hasHighWorkload) {
        if (a.region_type === RegionType.REPORTS || a.region_type === RegionType.ANALYTICS) return -1;
        if (b.region_type === RegionType.REPORTS || b.region_type === RegionType.ANALYTICS) return 1;
      }
    ...
    return optimized.map((region) => {
      ...
      if (isPeakHours && region.region_type === RegionType.SCHEDULING) {
  ...
  const getContextualBehavior = useCallback((regionId: string) => {
    ...
    if (region.region_type === RegionType.SCHEDULING) {
      shouldExpand = isPeakHours || hasHighWorkload;
      priority = isPeakHours ? 10 : 5;
    }

    if (region.region_type === RegionType.REPORTS || region.region_type === RegionType.ANALYTICS) {
    ...
    if (userRole === 'technician') {
      if (region.region_type === RegionType.SETTINGS) {
        visibility = 'hidden';
      }
    }
