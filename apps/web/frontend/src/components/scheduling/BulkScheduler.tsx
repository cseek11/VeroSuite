...
import type { Job as BaseJob } from '@/types/enhanced-types';

type JobWithRelations = BaseJob & {
  customer?: {
    id?: string;
    name?: string;
  };
...
export default function BulkScheduler({ 
  ...
  const [_pendingAssignments, setPendingAssignments] = useState<Array<{
    jobId: string;
    technicianId: string;
  }>>([]);
  ...
  const { data: jobs = [], isLoading: jobsLoading } = useQuery<JobWithRelations[]>({
    ...
    queryFn: async () => {
      const dateStr: string = selectedDate.toISOString().split('T')[0] ?? new Date().toISOString().substring(0, 10);
  ...
  const filteredJobs = useMemo(() => {
    return jobs.filter((job: JobWithRelations) => {
  ...
  const checkBulkConflicts = async () => {
    ...
    const jobsToCheck = filteredJobs.filter((job: JobWithRelations) => selectedJobs.has(job.id));
  ...
  // Select all jobs
  const selectAllJobs = () => {
    setSelectedJobs(new Set(filteredJobs.map((job: JobWithRelations, _index: number) => job.id)));
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="mb-6">
        <ScheduleCalendar
          ...
          filterPriority="all"
          {...(selectedTechnician ? { filterTechnician: selectedTechnician } : {})}
            ...
            <div className="p-4">
              {jobsLoading ? (
                ...
                <div className="space-y-2">
                  {filteredJobs.map((job: JobWithRelations) => (
                    <div
                      key={job.id}
      ...
      {conflictData && (
        <ConflictResolutionDialog
          ...
          {...(() => {
            ...
            const name = `${selectedTechnicianData.first_name ?? ''} ${selectedTechnicianData.last_name ?? ''}`.trim();
