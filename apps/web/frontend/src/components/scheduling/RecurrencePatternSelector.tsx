...
export interface RecurrencePattern {
  ...
  recurrence_days_of_week?: number[] | undefined; // 0=Sunday, 6=Saturday
  recurrence_day_of_month?: number | undefined; // 1-31
  recurrence_weekday_of_month?: string | undefined; // 'first_monday', 'last_friday', etc.
  start_date: string; // ISO date string
  end_date?: string | undefined; // ISO date string
...
export const RecurrencePatternSelector: React.FC<RecurrencePatternSelectorProps> = ({
  ...
  const [pattern, setPattern] = useState<RecurrencePattern>(() => {
    const basePattern: RecurrencePattern = {
      ...
      start_date: startDate.toISOString().split('T')[0] ?? new Date().toISOString().substring(0, 10),
      end_date: endDate?.toISOString().split('T')[0] ?? undefined,
    };
    return value ? { ...basePattern, ...value } : basePattern;
  });
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value: string) =>
              handleChange({ recurrence_type: value as RecurrencePattern['recurrence_type'] })
            }
          >
            <SelectTrigger>
              <SelectValue />
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <Switch
              id="has-end-date"
              checked={pattern.end_date !== undefined}
              onCheckedChange={(checked) =>
                handleChange({ end_date: checked ? pattern.end_date ?? '' : undefined })
              }
            />
          ...
          <div className="flex items-center gap-2">
            <Switch
              ...
              checked={pattern.max_occurrences !== undefined}
              onCheckedChange={(checked) =>
                handleChange({ max_occurrences: checked ? pattern.max_occurrences ?? 1 : undefined })
              }
            />
