  ...
  return (
    ...
    >
      ...
      <div className="mobile-header bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex-1 text-center">
          <h2 className="text-lg font-semibold text-gray-900">
            {currentRegion ? currentRegion.region_type.replace(/-/g, ' ') : 'No region'}
          </h2>
          <p className="text-xs text-gray-500">
            {visibleRegions.length > 0 ? `${currentIndex + 1} of ${visibleRegions.length}` : '0 of 0'}
          </p>
        </div>
      ...
      >
        ...
        >
          {visibleRegions.map((region, _index) => (
            <div
              key={region.id}
