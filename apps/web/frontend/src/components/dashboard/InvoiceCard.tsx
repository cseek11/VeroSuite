...
import type { CreateInvoiceDto } from '@/types/enhanced-types';
...
export default function InvoiceCard({ 
  ...
  const handleCreateInvoice = useCallback(async (customer: Record<string, unknown>, job?: Record<string, unknown>): Promise<void> => {
    ...
    try {
      ...
      const issueDate: string = new Date().toISOString().split('T')[0] ?? new Date().toISOString().substring(0, 10);
      const dueDate: string = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] ?? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString().substring(0, 10);
      const invoiceData: CreateInvoiceDto = {
        account_id: String(customer.id || ''),
        ...(job?.id ? { job_id: String(job.id) } : {}),
        issue_date: issueDate,
        due_date: dueDate,
        items: Array.isArray(job?.items) && job.items.length > 0
          ...
          : [{
              service_type_id: '',
              description: String(job?.description || 'Service'),
              quantity: 1,
              unit_price: typeof job?.total_amount === 'number' ? job.total_amount : 0
            }],
    ...
    try {
      if (payload.sourceDataType === 'customer') {
        ...
        try {
          ...
          logger.error('Failed to create invoice from job drag', {
