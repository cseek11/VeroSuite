...
async function fetchActiveSessions(userId: string): Promise<ActiveSession[]> {
  ...
  let response;
  try {
    response = await fetch(`${API_BASE_URL}/v1/users/${userId}/sessions`, {
      method: 'GET',
      headers: {
...
async function revokeSession(userId: string, sessionId: string): Promise<void> {
  ...
  let response;
  try {
    response = await fetch(`${API_BASE_URL}/v1/users/${userId}/sessions/${sessionId}`, {
    method: 'DELETE',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`,
  ...
  if (!response.ok) {
    ...
    logger.error('Failed to revoke user session', {
      error: error instanceof Error ? error.message : String(error),
      userId,
      sessionId
    });
