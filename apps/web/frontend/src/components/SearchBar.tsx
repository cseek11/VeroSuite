...
export const SearchBar: React.FC<SearchBarProps> = ({
  ...
  const handleInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim()) {
      ...
      try {
        ...
        logger.error('Search failed in input change handler', {
          error: error instanceof Error ? error.message : String(error),
          searchTerm: value
        });
        // Error state is handled by the search integration hook
      }
    } else {
      setIsOpen(false);
    }
  ...
  const handleAccountSelect = (account: Account) => {
    ...
    addToRecentSearches(searchResult as any);
    setInputValue('');
    setIsOpen(false);
    setSelectedIndex(-1);
  ...
  const handleSearch = async (term: string) => {
    ...
    try {
      ...
      logger.error('Search failed in search handler', {
        error: error instanceof Error ? error.message : String(error),
        searchTerm: term
      });
      // Error state is handled by the search integration hook
