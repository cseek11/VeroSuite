...
import { TechnicianStatus, EmploymentType, TechnicianQueryParams } from '../../types/technician';

type TechnicianFilters = TechnicianQueryParams & { page: number; limit: number };
...
const [filters, setFilters] = useState<TechnicianFilters>(initialFilters);
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    setFilters((prev: TechnicianFilters) => ({ ...prev, page: 1 }));
    refetch();
  };

  const handleFilterChange = (key: string, value: string) => {
    setFilters((prev: TechnicianFilters) => {
      if (key === 'status') {
        const next = { ...prev, page: 1 };
        if (value) {
          return { ...next, status: value as TechnicianStatus };
        }
        const { status, ...rest } = next;
        return rest as TechnicianFilters;
      }
      if (key === 'employment_type') {
        const next = { ...prev, page: 1 };
        if (value) {
          return { ...next, employment_type: value as EmploymentType };
        }
        const { employment_type, ...rest } = next;
        return rest as TechnicianFilters;
      }
      return { ...prev, [key]: value, page: 1 } as TechnicianFilters;
    });
  };
        ...
        {data && data.total_pages > 1 && (
          <div className="flex items-center justify-between px-6 py-3 border-t border-gray-200">
            ...
            <div className="flex space-x-2">
              <Button
                onClick={() => setFilters((prev: TechnicianFilters) => ({ ...prev, page: (prev.page ?? 2) - 1 }))}
              ...
              <Button
                onClick={() => setFilters((prev: TechnicianFilters) => ({ ...prev, page: (prev.page ?? 0) + 1 }))}
                disabled={(filters.page ?? 0) >= data.total_pages}
                variant="outline"
                size="sm"
              >
                Next
