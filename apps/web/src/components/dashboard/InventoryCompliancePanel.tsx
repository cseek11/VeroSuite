...
import type { ComplianceAlert, Inspection, InventoryCategory } from '@/types/inventory';
  ...
  const { data: inventoryCategories = [] } = useQuery({
    ...
    queryFn: async (): Promise<InventoryCategory[]> => {
      if (enhancedApi.inventory && typeof enhancedApi.inventory.getCategories === 'function') {
        const categories = await enhancedApi.inventory.getCategories();
  ...
  const { data: complianceAlerts = [] } = useQuery({
    ...
    queryFn: async (): Promise<ComplianceAlert[]> => {
      if (enhancedApi.inventory && typeof enhancedApi.inventory.getComplianceAlerts === 'function') {
        const alerts = await enhancedApi.inventory.getComplianceAlerts();
        // Map to ensure type compatibility with ComplianceAlert
        return alerts.map((alert: any) => ({
          id: String(alert.id || ''),
          type: alert.type as 'expiring' | 'low_stock' | 'out_of_stock' | 'compliance_issue',
  ...
  const { data: recentInspections = [] } = useQuery({
    ...
    queryFn: async (): Promise<Inspection[]> => {
      if (enhancedApi.inventory && typeof enhancedApi.inventory.getRecentInspections === 'function') {
        const inspections = await enhancedApi.inventory.getRecentInspections();
        // Map to match the expected Inspection type structure
        return inspections.map((insp: Inspection) => ({
          ...insp,
            ...
            <div key={index} className="border rounded-lg p-4">
              <div className="flex justify-between items-center mb-3">
                ...
                <div className="flex items-center space-x-2">
                  <Badge variant="default" className={category.compliance && category.compliance >= 95 ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${category.compliance && category.compliance >= 95 ? 'bg-green-500' : 'bg-yellow-500'}`}
                ...
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="text-right">
                    <Heading level={4} className="font-bold text-green-600">
                      {inspection.result === 'pass' ? '100%' : inspection.result === 'fail' ? '0%' : '50%'}
                    </Heading>
