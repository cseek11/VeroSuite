...
async function fetchUserHierarchy(userId: string): Promise<UserHierarchyData> {
  ...
  let response;
  try {
    response = await fetch(`${API_BASE_URL}/v1/users/${userId}/hierarchy`, {
      method: 'GET',
      headers: {
    ...
    logger.error('Failed to fetch user hierarchy', {
...
export default function UserHierarchy({ userId, onUserClick }: UserHierarchyProps) {
  const { data: hierarchy, isLoading, error } = useQuery<UserHierarchyData>({
