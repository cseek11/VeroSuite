...
async function fetchCertificationAlerts(daysAhead: number = 30): Promise<CertificationAlert[]> {
  ...
  let response;
  try {
    response = await fetch(`${API_BASE_URL}/v1/users/certifications/alerts?daysAhead=${daysAhead}`, {
      method: 'GET',
      headers: {
    ...
    logger.error('Failed to fetch certification alerts', {
...
export default function CertificationAlerts() {
  ...
  const { data: alerts = [], isLoading, error } = useQuery<CertificationAlert[]>({
