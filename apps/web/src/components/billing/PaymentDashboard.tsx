...
import type { PaymentTrackingData, PaymentAnalyticsData, ARSummaryData } from '@/types/api.types';
...
export default function PaymentDashboard() {
  ...
  const { data: trackingData, isLoading: isLoadingTracking, error: trackingError, refetch: refetchTracking } = useQuery<PaymentTrackingData>({
  ...
  const { data: analyticsData, isLoading: isLoadingAnalytics, error: analyticsError } = useQuery<PaymentAnalyticsData>({
    queryKey: ['payment-analytics', startDate, endDate],
    queryFn: async (): Promise<PaymentAnalyticsData> => {
      const result = await billing.getPaymentAnalytics(startDate ?? '', endDate ?? '');
      return result as PaymentAnalyticsData;
    },
  });
  ...
  // Fetch AR summary
  const { data: arSummaryData, isLoading: isLoadingAR } = useQuery<ARSummaryData>({
    queryKey: ['billing', 'ar-summary'],
    queryFn: async (): Promise<ARSummaryData> => {
      const result = await billing.getARSummary();
      // Convert ARSummary to ARSummaryData format
      return {
        totalAR: result.totalAR ?? 0,
        overdueAR: result.agingBuckets?.reduce((sum, bucket) => sum + (bucket.amount || 0), 0),
        currentAR: result.totalAR ?? 0,
      };
    },
  });
