...
import type { ReminderHistory } from '@/types/api.types';
...
export default function InvoiceReminders({ onReminderSent }: InvoiceRemindersProps) {
  ...
  const handleSelectAll = () => {
    if (selectedInvoices.size === filteredInvoices.length) {
      ...
      setSelectedInvoices(new Set(filteredInvoices.map((inv: Invoice) => inv.id ?? '').filter(id => id !== '')));
    }
  };
            ...
            <>
              ...
              <div className="space-y-3">
                {filteredInvoices.map((invoice: Invoice) => {
                  ...
                  return (
                    <Card
                      key={invoice.id ?? `invoice-${Math.random()}`}
