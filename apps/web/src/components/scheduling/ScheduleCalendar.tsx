...
export type { Job } from '@/types/enhanced-types';

// Types
// Minimal technician shape used in scheduling views
...
export const ScheduleCalendar: FC<ScheduleCalendarProps> = ({
  ...
  const [_editingJob, setEditingJob] = useState<JobWithRelations | null>(null);
  ...
  const handleDrop = async (event: DragEvent, targetDate: Date, targetTime: string) => {
    ...
    const newEndTimeStr = newEndTime.toTimeString().split(' ')[0]?.substring(0, 5) ?? '10:00';
      ...
      <div className="bg-white rounded-md shadow-sm relative">
            ...
            {selectedJob && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">
                    {selectedJob.service?.description || (selectedJob as any).job_description || 'No description'}
                  </p>
                </div>
              </div>
            )}
        ...
        {conflictData && (
          <ConflictResolutionDialog
            ...
            {...(() => {
              if (!draggedJob?.technician_id) return {};
              const technician = technicians.find((t: any) => (t.id || t.user_id) === draggedJob.technician_id);
              if (!technician) return {};
              const technicianName = `${technician.first_name ?? ''} ${technician.last_name ?? ''}`.trim();
              return technicianName ? { technicianName } : {};
            })()}
          />
        )}
...
const JobCreateDialog: FC<JobCreateDialogProps> = ({
  ...
  const {
    ...
    defaultValues: {
      ...
      service_type: '',
      priority: 'medium',
      scheduled_date: selectedDate.toISOString().split('T')[0] ?? new Date().toISOString().substring(0, 10),
  ...
  useEffect(() => {
    if (open) {
      ...
      const safeDate: string =
        baseDate.toISOString().split('T')[0] ?? new Date().toISOString().split('T')[0] ?? '';
  ...
  const createJobMutation = useMutation({
    mutationFn: async (jobData: JobCreateFormData) => {
      if (isRecurring && recurrencePattern) {
        ...
        const template = await enhancedApi.jobs.recurring.createTemplate(templatePayload);

        // Generate jobs for the next 3 months
        const generateUntil = new Date();
        generateUntil.setMonth(generateUntil.getMonth() + 3);
        const generateUntilStr = generateUntil.toISOString().split('T')[0] ?? new Date().toISOString().substring(0, 10);
        await enhancedApi.jobs.recurring.generate(template.id, generateUntilStr);
        ...
        return await enhancedApi.jobs.create(jobPayload);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['jobs'] });
                  ...
                  <>
                    ...
                    {!watchedCustomerId ? (
                      ...
                      <Input
                        ...
                        placeholder="No locations found - enter location ID"
                        error={errors.location_id?.message}
                      />
                    )}
                    {errors.location_id && (
