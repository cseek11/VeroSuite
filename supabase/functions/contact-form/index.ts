...
serve(async (req) => {
  ...
  try {
    ...
    console.log('reCAPTCHA validation check:', { 
    ...
    if (recaptchaToken && recaptchaSecretKey) {
      try {
        ...
        console.log('reCAPTCHA API response:', recaptchaResult)
        
        if (!recaptchaResult.success || recaptchaResult.score < 0.3) {
          console.log('reCAPTCHA validation failed:', recaptchaResult)
          // For now, let's be more lenient and just log the failure but continue
          console.log('reCAPTCHA failed but continuing with honeypot protection')
        } else {
          console.log('reCAPTCHA validation passed, score:', recaptchaResult.score)
        }
      } catch (recaptchaError) {
        console.error('reCAPTCHA API error:', recaptchaError)
        console.log('reCAPTCHA API error but continuing with honeypot protection')
      }
    ...
    const confirmationEmailContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="text-align: center; margin-bottom: 30px;">
          <h1 style="margin: 0; font-size: 28px;">Welcome to <span style="color: #7c3aed;">Vero</span><span style="color: #10b981;">Field</span>!</h1>
        ...
        <div style="text-align: center; color: #6b7280; font-size: 12px;">
          ...
          <div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #e5e7eb;">
            <p style="margin: 5px 0;">Don't want to receive these emails?</p>
            <p style="margin: 5px 0;">
              <a href="https://iehzwglvmbtrlhdgofew.supabase.co/functions/v1/unsubscribe?email=${encodeURIComponent(sanitizedData.email)}&token=${leadData?.[0]?.id}" 
    ...
    const sendEmail = async (emailData: any, emailType: string, retries = 2) => {
      for (let attempt = 1; attempt <= retries; attempt++) {
        try {
          console.log(`Sending ${emailType} email (attempt ${attempt}/${retries}) to:`, emailData.personalizations[0].to[0].email)
          ...
          if (!response.ok) {
            ...
            console.error(`SendGrid ${emailType} error (attempt ${attempt}):`, errorText)
            console.error(`SendGrid ${emailType} status:`, response.status)
            
            if (attempt === retries) {
              console.error(`SendGrid ${emailType} failed after ${retries} attempts`)
              return false
            }
          ...
          console.error(`Error sending ${emailType} (attempt ${attempt}):`, error)
          
          if (attempt === retries) {
            console.error(`SendGrid ${emailType} failed after ${retries} attempts due to network error`)
            return false
          }
    ...
    // If notification email failed, log detailed info for manual follow-up
    if (!notificationSent) {
      console.error('NOTIFICATION EMAIL FAILED - Manual follow-up required:', {
        leadId: leadData?.[0]?.id,
        email: sanitizedData.email,
        name: `${sanitizedData.first_name} ${sanitizedData.last_name}`,
        company: sanitizedData.company,
        timestamp: sanitizedData.user_timestamp,
        timezone: sanitizedData.user_timezone
      })
    }
